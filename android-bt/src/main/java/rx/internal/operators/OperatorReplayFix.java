...
public final class OperatorReplayFix<T> extends ConnectableObservable<T> {
  ...
  /**
   * Creates a OperatorReplayFix instance to replay values of the given source observable.
  ...
  private OperatorReplayFix(OnSubscribe<T> onSubscribe, Observable<? extends T> source,
      final AtomicReference<ReplaySubscriber<T>> current,
      final Func0<? extends ReplayBuffer<T>> bufferFactory) {
    super(onSubscribe);
    this.source = source;
